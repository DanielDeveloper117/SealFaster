...
try {
    ...
    $sqlObtener = "SELECT id, img_envio_barras, img_envio_paquete, img_recepcion_barras, img_recepcion_paquete 
                   FROM operaciones_inv 
                   WHERE id = :id AND tipo = 'Traspaso'";
    ...
    if (!$operacion) {
        ...
        echo json_encode(['success' => false, 'error' => 'El traspaso no existe o no es un traspaso válido']);
        exit;
    }
    ...
    try {
        ...
        $archivos_a_eliminar = [
            ...
            $operacion['img_recepcion_barras'],
            $operacion['img_recepcion_paquete']
        ];
        ...
        echo json_encode([
            'success' => false, 
            'error' => 'Error al eliminar el traspaso: ' . $e->getMessage()
        ]);
    }
