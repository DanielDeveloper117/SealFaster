$(document).ready(function(){
    ...
    // 1. DECLARACIÓN ÚNICA: Asegúrate de que esto esté en el scope global (fuera de document.ready si es posible)
    // o al menos fuera de cualquier función de click.
    ...
    $("#buscadorCotizaciones").on("change", function () {
        ...
        // Usamos String() para normalizar el ID, ya que a veces viene como number desde el data-id
        ...
        // Ocultar la opción en el selector para que no se repita visualmente
        $option.addClass('d-none');

        // 2. CHECK DE DUPLICADOS: Normalizamos a String para comparar correctamente
        if (cotizacionesSeleccionadas.map(String).includes(id)) {
            ...
            // 3. ACTUALIZACIÓN DEL INPUT: Sincronizamos con el backend
            $('#inputCotizaciones').val(cotizacionesSeleccionadas.join(', '));
        }

        // 4. RESET DEL CHOSEN: Limpiamos el buscador para la siguiente selección
    ...
    $(document).on('click', '.btnEliminarFila', function () {
        ...
        // Usamos String() para asegurar que la comparación sea entre tipos iguales
        cotizacionesSeleccionadas = cotizacionesSeleccionadas.filter(id => String(id) !== String(idAEliminar));
        
        // 3. ACTUALIZACIÓN CRÍTICA:
        // Si el arreglo queda vacío, ponemos string vacío, si no, los unimos
        const nuevoValor = cotizacionesSeleccionadas.length > 0 ? cotizacionesSeleccionadas.join(', ') : "";
        $('#inputCotizaciones').val(nuevoValor);

        // Debug para el desarrollador
        console.log("IDs restantes:", nuevoValor);
    ...
    $('#productionTable').on('click', '.edit-btn', function() {
        ...
        // Mostramos un loader o bloqueamos el botón momentáneamente
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Llenamos campos simples
                    $('#inputIdRequisicion').val(r.id_requisicion);
                    $('#inputFolio').val(r.folio);
                    ...
                    const idsString = response.requisicion.cotizaciones || "";
                    // Convertimos el string "1, 2" en un array de strings limpios
                    cotizacionesSeleccionadas = idsString.split(',')
                                                .map(s => s.trim())
                                                .filter(s => s !== "");
                            // Sincronizamos el input oculto de inmediato
                    $('#inputCotizaciones').val(cotizacionesSeleccionadas.join(', '));

                    // Actualizamos la tabla visual
                    actualizarTablaYSeleccion(response.cotizaciones_detalles);

                    // --- COORDINACIÓN DEL CHOSEN ---
                    // 1. Asegurarnos que las cotizaciones de esta req existan en el select
                    response.cotizaciones_detalles.forEach(function(cot) {
                        if ($(`#buscadorCotizaciones option[value="${cot.id_cotizacion}"]`).length === 0) {
                            // Si no existe en el Chosen (porque no estaba en las top 50), la agregamos
                            let nuevaOpcion = new Option(`${cot.id_cotizacion} - ${cot.perfil_sello}`, cot.id_cotizacion, false, false);
                            $(nuevaOpcion).attr({
                                'data-perfil': cot.perfil_sello,
                                'data-di': cot.di_sello,
                                'data-de': cot.de_sello,
                                'data-a': cot.a_sello
                                // agrega los demás data- que necesites
                            });
                            $('#buscadorCotizaciones').append(nuevaOpcion);
                        }
                    });

                    // 2. Actualizar Chosen y tabla visual
                    actualizarTablaYSeleccion(response.cotizaciones_detalles);
    ...
    function actualizarTablaYSeleccion(cotizaciones) {
        $('#miniTableCotizaciones tbody').empty();
        $('#buscadorCotizaciones option').removeClass('d-none'); // Reset
        
        cotizaciones.forEach(function(cot) {
            // Marcamos como "seleccionada" (oculta en el select según tu lógica)
            ...
            $('#miniTableCotizaciones tbody').append(`
                <tr>
                    ...
                    <td>${cot.di_sello}/${cot.de_sello}/${cot.a_sello}</td>
                </tr>
            `);
        });
        $('#buscadorCotizaciones').trigger("chosen:updated");
    ...
    // CLICK SUBMIT A Archivar LA PRODUCCION DE LA REQUISICION
