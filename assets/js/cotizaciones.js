    ...
    // cancelar agrupacion y recargar la página sin el parámetro de agrupación
    ...
    // ========== CARGAR CONTEO DE COTIZACIONES VENCIDAS ==========
    function cargarCotizacionesVencidas() {
        $.ajax({
            url: '../ajax/contar_cotizaciones_vencidas.php',
            type: 'POST',
            dataType: 'json',
            success: function(response) {
                if (response.success && response.total_vencidas > 0) {
                    $('#contadorVenciadas').text(response.total_vencidas);
                    $('#btnArchivarVenciadas').removeClass('d-none');
                } else {
                    $('#btnArchivarVenciadas').addClass('d-none');
                }
            },
            error: function(error) {
                console.error('Error al cargar cotizaciones vencidas:', error);
            }
        });
    }
    // ============================================================
    ...
    $(document).ready(function () {
        ...
        // Cargar conteo de vencidas al iniciar
        if (cotActual === 'u') {
            cargarCotizacionesVencidas();
        }
        ...
        // ========== ARCHIVAR TODAS LAS COTIZACIONES VENCIDAS ==========
        $("#btnArchivarVenciadas").on('click', function() {
            var totalVenciadas = $('#contadorVenciadas').text();
            
            Swal.fire({
                title: '¿Confirmar acción?',
                html: 'Esta acción archivará <strong>' + totalVenciadas + '</strong> cotizaciones vencidas y mejorará la carga de datos.<br><br>¿Desea continuar?',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    Swal.fire({
                        title: 'Procesando...',
                        html: 'Archivando cotizaciones vencidas',
                        icon: 'info',
                    ...
                    $.ajax({
                        url: '../ajax/archivar_venciadas.php',
                        type: 'POST',
                        dataType: 'json',
                        success: function(response) {
                            if (response.success) {
                                Swal.fire({
                                    title: '¡Éxito!',
                                    html: 'Se archivaron <strong>' + response.registros_archivados + '</strong> cotizaciones vencidas exitosamente.',
                                    icon: 'success',
                                ...
                                Swal.fire(
                                    'Error',
                                    response.error || 'Ocurrió un error al archivar las cotizaciones',
                                    'error'
                                );
                            }
                        },
                        error: function(xhr, status, error) {
                            ...
                            Swal.fire(
                                'Error',
                                'Error al realizar la petición: ' + error,
                                'error'
                            );
                        }
                    });
        ...
        function detectarZoom() {
            ...
            if (anchoPantalla <= 991) {
                ...
                $("#btnArchivarVenciadas").html('<i class="bi bi-archive"></i>');
