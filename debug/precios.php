...
if (isset($_GET['material']) && !empty($_GET['material'])) {
    $material = trim($_GET['material']); // Limpiamos espacios

    $sqlPrecios = "SELECT * FROM parametros WHERE material LIKE :material";
    $stmtPrecios = $conn->prepare($sqlPrecios);

    // USAR la variable correcta que recibiste arriba
    $busqueda = "%" . $material . "%"; 

    $stmtPrecios->execute([':material' => $busqueda]);
    $arregloSelectPrecios = $stmtPrecios->fetchAll(PDO::FETCH_ASSOC);
    // Limpiar CUALQUIER residuo de codificación extraña (Binary to String)
    foreach ($arregloSelectPrecios as &$row) {
        foreach ($row as $key => $value) {
            if (is_string($value)) {
                // Elimina caracteres no imprimibles y bytes nulos
                $row[$key] = preg_replace('/[\x00-\x1F\x7F-\xFF]/', '', $value);
                // Si el problema es UTF-16 leído como UTF-8, esto lo "aplana":
                $row[$key] = str_replace("\0", "", $row[$key]);
            }
        }
    }
