...
if (isset($_GET['id_requisicion'])) {
    ...
    foreach ($cotizacion_ids as $id_cotizacion) {
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(30, 6, 'Perfil', 1, 0, 'C', true);
        $pdf->Cell(31, 6, 'Familia', 1, 0, 'C', true);
        $pdf->Cell(30, 6, 'Medida indicada', 1, 0, 'C', true);
        $pdf->Cell(33, 6, 'D. Interior', 1, 0, 'C', true);
        $pdf->Cell(33, 6, 'D. Exterior', 1, 0, 'C', true);
        if($familiaPerfil=="wipers"){
            $pdf->Cell(33, 6, 'Altura total', 1, 1, 'C', true);
        }else{
            $pdf->Cell(33, 6, 'Altura', 1, 1, 'C', true);
        }

        $pdf->SetFont('Arial', '', 10);
        $pdf->Cell(30, 6, utf8_decode($cot['perfil_sello']), 1, 0, 'C');
        $pdf->Cell(31, 6, utf8_decode($familiPerfilR), 1, 0, 'C');
        $pdf->Cell(30, 6, utf8_decode($cot['tipo_medida']), 1, 0, 'C');


        if($cot['tipo_medida']=="Sello"){
            $pdf->Cell(33, 6, utf8_decode($cot['di_sello'].'mm/'.$cot['di_sello_inch'].'"'), 1, 0, 'C');
            $pdf->Cell(33, 6, utf8_decode($cot['de_sello'].'mm/'.$cot['de_sello_inch'].'"'), 1, 0, 'C');
            $pdf->Cell(33, 6, utf8_decode($cot['a_sello'].'mm/'.$cot['a_sello_inch'].'"'), 1, 1, 'C');

        }else{
            $pdf->Cell(33, 6, utf8_decode($cot['di_sello2'].'mm/'.$cot['di_sello_inch2'].'"'), 1, 0, 'C');
            $pdf->Cell(33, 6, utf8_decode($cot['de_sello2'].'mm/'.$cot['de_sello_inch2'].'"'), 1, 0, 'C');
            $pdf->Cell(33, 6, utf8_decode($cot['a_sello2'].'mm/'.$cot['a_sello_inch2'].'"'), 1, 1, 'C');
        }

        // ******************************LAS OTRAS ALTURAS************************************************
        if($familiaPerfil=="wipers"){
            $pdf->Cell(30, 6, '', 1, 0, 'C', true);
            $pdf->Cell(31, 6, '', 1, 0, 'C', true);
            $pdf->Cell(30, 6, '', 1, 0, 'C', true);
        }
        ...
        if($esWisper !== "0" && $conEscalon == "0" && $wisperEspecial == "0"){
            $pdf->Cell(33, 6, 'Altura caja', 1, 1, 'C', true);
        }
        if($esWisper !== "0" && $conEscalon == "0" && $wisperEspecial !== "0"){
            $pdf->Cell(33, 6, 'Altura caja', 1, 0, 'C', true);
        }
        // altura de caja con escalon
        if($esWisper !== "0" && $conEscalon !== "0"){
            $pdf->Cell(33, 6, 'Altura caja', 1, 0, 'C', true);
            $pdf->Cell(33, 6, utf8_decode('Altura escalón'), 1, 1, 'C', true);
        }
        // alturas wisper especial
        if($wisperEspecial !== "0"){
            $pdf->Cell(33, 6, 'Altura H2', 1, 0, 'C', true);
            $pdf->Cell(33, 6, 'Altura H3', 1, 1, 'C', true);
        }
        if($familiaPerfil=="wipers"){
            $pdf->Cell(30, 6, '', 1, 0, 'C');
            $pdf->Cell(31, 6, '', 1, 0, 'C');
        ...
        if($esWisper !== "0" && $conEscalon == "0" && $wisperEspecial == "0"){
            ...
            $pdf->Cell(33, 6, utf8_decode($cot['altura_caja'].'mm/'.mm_a_pulgadas($cot['altura_caja']).'"'), 1, 1, 'C');
        }
        if($esWisper !== "0" && $conEscalon == "0" && $wisperEspecial !== "0"){
            if ($cot['altura_caja'] === "0.00") {
                $pdf->SetFont('Arial', 'I', 10);
            ...
            $pdf->Cell(33, 6, utf8_decode($cot['altura_caja'].'mm/'.mm_a_pulgadas($cot['altura_caja']).'"'), 1, 0, 'C');
        }
        // ------------- altura de caja con escalon
        if($esWisper !== "0" && $conEscalon !== "0"){
            if ($cot['altura_caja'] === "0.00") {
            ...
            $pdf->Cell(33, 6, utf8_decode($cot['altura_escalon'].'mm/'.mm_a_pulgadas($cot['altura_escalon']).'"'), 1, 1, 'C');
        }
        // ------------ alturas wisper especial
        if($wisperEspecial !== "0"){
            ...
            $pdf->Cell(33, 6, utf8_decode($cot['altura_h2'].'mm/'.mm_a_pulgadas($cot['altura_h2']).'"'), 1, 0, 'C');
            if ($cot['altura_h3'] === "0.00") {
                $pdf->SetFont('Arial', 'I', 10);
            } else {
                $pdf->SetFont('Arial', '', 10);
            }
            $pdf->Cell(33, 6, utf8_decode($cot['altura_h3'].'mm/'.mm_a_pulgadas($cot['altura_h3']).'"'), 1, 1, 'C');
        }
        // ***********************************************************************************************
        ...
        $pdf->Cell(30, 6, 'Cantidad Pz.', 1, 0, 'C', true);
        $pdf->Cell(31, 6, 'Material', 1, 0, 'C', true);
        $pdf->Cell(129, 6, 'Claves', 1, 1, 'C', true);
        ...
        foreach ($cotizacionData as $cot) {
            ...
            $pdf->Cell(30, $maxRowHeight, $cot['cantidad'] . " pz", 1, 0, 'C');
            $pdf->Cell(31, $maxRowHeight, utf8_decode($cot['material']), 1, 0, 'C');
            // Guardamos la posición antes de usar MultiCell en Claves
            $x = $pdf->GetX();
            $y = $pdf->GetY();
            // Usamos MultiCell para Claves (esto puede generar varias líneas)
            $pdf->MultiCell(129, $lineHeight, $clavesVertical, 1, 'C');
