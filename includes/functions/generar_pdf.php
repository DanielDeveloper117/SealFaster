...
if (isset($_GET['id_cotizacion'])) {
    ...
    if (!empty($arregloCotizacion)) {
        ...
        $pdf->Cell(24, 6, utf8_decode($arregloCotizacion[0]["di_sello"]), 1, 0, 'C');
        $pdf->Cell(24, 6, utf8_decode($arregloCotizacion[0]["de_sello"]), 1, 0, 'C');
        // altura normal total
        if($esWisper !== "0" || $conEscalon !== "0"){
            $pdf->Cell(24, 6, utf8_decode($arregloCotizacion[0]["a_sello"]), 1, 0, 'C');
        }else{
            $pdf->Cell(24, 6, utf8_decode($arregloCotizacion[0]["a_sello"]), 1, 1, 'C');
        }
        // altura de caja para solo wispers sin escalon
        if($esWisper !== "0" && $conEscalon == "0" && $wisperEspecial == "0"){
